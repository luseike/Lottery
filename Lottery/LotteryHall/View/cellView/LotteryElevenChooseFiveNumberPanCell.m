//
//  LotteryElevenChooseFiveNumberPanCell.m
//  Lottery
//
...
@interface LotteryElevenChooseFiveNumberPanCell()
...
@implementation LotteryElevenChooseFiveNumberPanCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        for (NSInteger i = 1 ; i < 12; i++) {
            ...
            [numberBtn setTitle:[NSString stringWithFormat:@"%02zd",i] forState:UIControlStateNormal];
...
-(void)numberBtnClick:(UIButton *)numberBtn{
    ...
    if ([self.customDelegate respondsToSelector:@selector(ElevenChooseFiveNumberPanCell:didSelectedNumberBtn:)]) {
        [self.customDelegate ElevenChooseFiveNumberPanCell:self didSelectedNumberBtn:numberBtn];
    }
}
-(void)seperateBtnClick:(UIButton *)seperateBtn{
    if ([self.customDelegate respondsToSelector:@selector(ElevenChooseFiveNumberPanCell:didSelectedSeperateBtn:)]) {
        [self.customDelegate ElevenChooseFiveNumberPanCell:self didSelectedSeperateBtn:seperateBtn];
    }
}
...
-(void)setNumberPanModel:(LotteryNumberPanModel *)numberPanModel{
    ...
    if (numberPanModel.selectStrings.length > 0) {
        ...
        NSArray *selectedNumbers = [numberPanModel.selectStrings componentsSeparatedByString:@" "];
        for (NSInteger j = 0; j < self.numberBtnsView.subviews.count; j++) {
            UIButton *btn = [self.numberBtnsView.subviews objectAtIndex:j];
            
            for (NSString *selectNumber in selectedNumbers) {
                if ([selectNumber isEqualToString:btn.currentTitle]) {
                    btn.selected = YES;
...
-(void)layoutSubviews{
    ...
    self.numberBtnsView.height = numberBtnH * 3 + numberBtnRowMargin * 2;
