...
@interface Lottery18NumberPanCell()
...
@implementation Lottery18NumberPanCell
...
-(void)setIsFromOne:(BOOL)isFromOne{
    if (isFromOne) {
        ...
        [[self.numberBtnsView.subviews lastObject] removeFromSuperview];
//
...
-(void)numberBtnClick:(UIButton *)numberBtn{
    ...
    if ([self.number18PanCellDelegate respondsToSelector:@selector(NumberPanCell:didSelectedNumberBtn:)]) {
        [self.number18PanCellDelegate NumberPanCell:self didSelectedNumberBtn:numberBtn];
    }
}

-(void)seperateBtnClick:(UIButton *)seperateBtn{
    if ([self.number18PanCellDelegate respondsToSelector:@selector(NumberPanCell:didSelectedSeperateBtn:)]) {
        [self.number18PanCellDelegate NumberPanCell:self didSelectedSeperateBtn:seperateBtn];
    }
}
...
-(void)layoutSubviews{
    ...
    self.numberBtnsView.height = numberBtnH * 4 + numberBtnRowMargin * 3;
