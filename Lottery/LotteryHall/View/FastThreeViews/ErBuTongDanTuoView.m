...
-(void)danMaNumberLabelClick:(UITapGestureRecognizer *)tapGesture{
    ...
    if (numberLabel.backgroundColor == [UIColor whiteColor]) {
        ...
        if (self.danMaSelectedNumbers.length > 0) {
            ...
            if (selectedDanMaArr.count == 1) {
                [SVProgressHUD showInfoWithStatus:@"二不同号最多只能选择1个胆码"];
                return;
            }
        }
...
//        self.danMaSelectedNumbers = [NSString appendObjStr:numberLabel.text toSelectedStr:self.danMaSelectedNumbers];
...
//        self.danMaSelectedNumbers = [NSString deleteObjStr:numberLabel.text fromSelectedStr:self.danMaSelectedNumbers];
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"ErBuTongDanTuoNumberViewClickNotification" object:nil userInfo:@{@"selectedNumbers":[NSString stringWithFormat:@"%@ %@",self.danMaSelectedNumbers.length > 0 ? self.danMaSelectedNumbers : @"",self.tuoMaSelectedNumbers.length > 0 ? self.tuoMaSelectedNumbers : @""]}];
}
